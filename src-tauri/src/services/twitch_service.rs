...
impl TwitchService {
        ...
        tokio::task::spawn(async move {
            ...
            match result {
                Ok(token_response) => {
                    ...
                    // Store token to file (primary storage method for Windows)
                    println!("[LOGIN] Storing token to secure file storage...");
                    match Self::store_token_to_file(&storable_token) {
                        Ok(_) => {
                            println!("[LOGIN] ✅ Token stored successfully to file!");
                            ...
                            match Self::load_token_from_file() {
                                Ok(_) => {
                                    println!("[LOGIN] ✅ Verified: Token can be retrieved from file");
                                }
                                Err(e) => {
                                    eprintln!("[LOGIN] ⚠️ WARNING: Token was stored but cannot be retrieved: {:?}", e);
                                }
                            }
                        ...
                        Err(e) => {
                            eprintln!("[LOGIN] ❌ Failed to store token: {:?}", e);
                            let _ = app_handle.emit("twitch-login-error", format!("Failed to store token: {}", e));
                        }
                    }
                }
                Err(e) => {
    ...
    pub async fn logout(_state: &AppState) -> Result<()> {
        ...
        println!("Logout complete - all tokens cleared");
        Ok(())
    }
