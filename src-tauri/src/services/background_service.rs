...
impl BackgroundService {
        ...
        *is_running = true;
        ...
        tokio::spawn(async move {
            ...
            match TwitchService::get_followed_streams(&app_handle_ws.state()).await {
                Ok(streams) => {
                    let channel_ids: Vec<String> = streams.iter()
                        .map(|s| s.user_id.clone())
                        .collect();
                    
                    if !channel_ids.is_empty() {
                        println!("🔌 Connecting WebSockets to {} channels for real-time monitoring", channel_ids.len());
                        let mut ws = ws_service.lock().await;
                        if let Err(e) = ws.connect_to_channels(channel_ids, &user_id, &token, app_handle_ws).await {
            ...
            while *is_running_claim.read().await {
                ...
                if auto_claim_enabled {
                    ...
                    let token = match DropsAuthService::get_token().await {
                        Ok(token) => token,
                        Err(e) => {
                            eprintln!("Could not get drops auth token for channel points farming: {}", e);
                            continue;
                        }
                    };
                    ...
                    match TwitchService::get_followed_streams(&app_handle_claim.state()).await {
                        Ok(live_streams) => {
                            ...
                            println!("Found {} live streams to check for channel points bonuses.", live_streams.len());
                            ...
                            for stream in &live_streams {
                                ...
                                match cps.check_and_claim_points(&stream.user_login, &token, auto_claim_enabled).await {
                                    Ok(Some(points)) => {
                                        println!("Successfully claimed {} bonus points for {}.", points, stream.user_name);
                                    }
                                    Ok(None) => {
                                        // No bonus available, this is normal
                                    }
                                    Err(e) => {
                                        eprintln!("Error checking/claiming points for {}: {}", stream.user_name, e);
                                    }
                                }
                            }
                        }
        ...
        tokio::spawn(async move {
            ...
            while *is_running_watch.read().await {
                ...
                if minutes_since_rotation >= ROTATION_INTERVAL_MINUTES {
                    ...
                    match TwitchService::get_followed_streams(&app_handle_watch.state()).await {
                        Ok(live_streams) => {
                            ...
                            let streams_to_watch: Vec<_> = live_streams.iter()
                                .take(2)
                                .collect();
                            ...
                            for stream in streams_to_watch {
                                ...
                                println!("🎬 Starting to watch {} for channel points", stream.user_name);
                                if let Err(e) = cps.start_watching_stream(channel_id, channel_login, &token).await {
                ...
                if !watching.is_empty() {
                    println!("📡 Sending minute-watched payloads for {} streams", watching.len());
    ...
    pub async fn get_channel_points_balances(&self) -> Vec<crate::models::drops::ChannelPointsBalance> {
        let cps = self.channel_points_service.lock().await;
