...
pub async fn connect_discord(app_state: State<'_, AppState>) -> Result<(), String> {
    DiscordService::connect(&app_state).await.map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn disconnect_discord() -> Result<(), String> {
    DiscordService::disconnect().await.map_err(|e| e.to_string())
}

#[tauri::command]
pub async fn set_idle_discord_presence(app_state: State<'_, AppState>) -> Result<(), String> {
    DiscordService::set_idle_presence(&app_state).await.map_err(|e| e.to_string())
}
...
pub async fn clear_discord_presence(state: State<'_, AppState>) -> Result<(), String> {
    DiscordService::clear_presence(&state).await.map_err(|e| e.to_string())
}
