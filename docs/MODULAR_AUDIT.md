...
- Drops endpoint history: the legacy `spade.twitch.tv/track` path no longer credits drops (it does still credit channel points, see 2.2); the documented working path for drops is the `sendSpadeEvents` GraphQL mutation. The split is symmetric: the GQL path does not credit channel points (verified 2026-06-11 by watching with farming off on a GQL-only heartbeat; zero points accrued and no bonus chests were offered), so the parity heartbeat reports the same minute on both paths.
...
Heartbeat reality (verified). Two watch-reporting paths coexist and each credits a different thing. The drops miner uses the `sendSpadeEvents` GraphQL mutation (`mining_service.rs:3489-3549`); its own comment states the legacy `spade.twitch.tv/track` endpoint still returns 204 and still credits channel points, but no longer credits drops. The inverse also holds: the GQL mutation credits drops but not channel points (verified 2026-06-11; a GQL-only core heartbeat earned zero points and was never offered bonus chests, since chest availability follows credited watch-minutes). The background channel-points farmer posts to the legacy endpoint (`channel_points_service.rs:19`, `SPADE_URL`), which is why farming credits points. Per decision 2 (revised 2026-06-11), the core parity heartbeat reports the same watched minute on both paths; neither path can retire while Twitch keeps the crediting split.
...
Watch reporting:
- Core: one minute-watched heartbeat for the channel the user is actually watching, only while it is actually playing, using the user's own credential, triggered by the player, reported on both watch pipes (`sendSpadeEvents` for drop progress, legacy spade track for channel-points crediting; see the heartbeat reality note in 2.2). In MultiNook, the working default is that only the focused or spotlit tile heartbeats, so core telemetry always says "this user is watching one channel", which is true. (Widening to all visible tiles is possible later; see remaining opens.)
...
- Current state: the player-driven heartbeat exists (`services/watch_heartbeat_service.rs`, Phase 3) and reports on both crediting paths as of 2026-06-11. Before the dual-path fix, watching in core with farming disabled earned no points, exactly the failure mode this section predicted.
...
- [x] Extract a single active-channel minute-watched heartbeat tied to the player: one channel, only while it is actually rendering and playing, using the user's own credential. (services/watch_heartbeat_service.rs; playback gated by player playing and pause events; offline and VOD self-guard via the live broadcast-id check)
- [x] Report the watched minute on both crediting paths. The first cut sent `sendSpadeEvents` only; owner runtime verification on 2026-06-11 showed zero points accrued and no bonus chests offered with farming off, because the GQL path credits drops but not channel points. The heartbeat now also posts the legacy spade track event (the verified points-crediting field set, with `location` and `player`).
...
- [ ] Confirm that with no plugins enabled, points accrue and drop progress advances on the watched channel only, and nothing is sent for any other channel. (re-verification pending after the dual-path change: needs a live watch session and balance observation by the owner)
...
- [x] The plugin does its own GQL and watch reporting over its own `reqwest` stack, using the `twitch.android` credential handed over through the broker on consent. It reports each watched minute on BOTH watch pipes: `sendSpadeEvents` (drop progress) and the legacy `spade.twitch.tv/track` endpoint (channel points). The earlier "standardize on `sendSpadeEvents`" plan was wrong: the GQL pipe does not credit channel points (verified 2026-06-11), so a points-farming sidecar that used it alone would advance drops but earn no points. It also excludes the core's actively-watched channel (received via `on_watch_tick`'s `active_channel_id`) so it never wastes a concurrent slot re-reporting what the core heartbeat already covers. Client ids: the public web client id is a constant for context reads; the Android client id comes from the credential handover.
...
- [ ] Confirm the core emits `on_followed_live`, `on_watch_tick`, and channel events (already wired in Phase 3) and contains none of the background mining loops or the multi-channel farming calls (the watched-channel heartbeat's own dual-pipe send stays).
- [ ] Do NOT retire the legacy `spade.twitch.tv/track` path: it is the only pipe that credits channel points. Both the core heartbeat and the farming sidecar send on it plus `sendSpadeEvents`. Only the multi-channel farming USE of it leaves core with the sidecar.
...
2. Heartbeat endpoint: originally "core standardizes on `sendSpadeEvents` and the legacy path retires". Revised 2026-06-11 after runtime verification: the GQL path does not credit channel points, so the parity heartbeat sends the same watched minute on both `sendSpadeEvents` (drops) and legacy `spade.twitch.tv/track` (points). The farming extraction still removes the multi-channel legacy use; the single-channel legacy send stays in core as part of the heartbeat.
